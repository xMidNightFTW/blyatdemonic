	...
	else if (g_Engine->GetPlayerInfo(pPlayer->GetIndex(), &info))
	{
		...
		if (weapon)
		{
			...
			if (g_Options.Visuals.Armor && weapon->IsInReload())
				texts.push_back(Text("Bullets Remains", 2, g_Render->font.ESP, Color(121, 252, 250, 255)));
			if (g_Options.Visuals.Armor)
			...
			if (g_Options.Visuals.ammo)
			{
				...
				if (MiscFunctions::IsKnife(pWeapon) || MiscFunctions::IsGrenade(pWeapon))
				{
					...
					texts.push_back(Text(std::string("Remain: 0"), 1, g_Render->font.ESPMini, Color(255, 255, 255, 255)));
				}
				else
				{
					texts.push_back(Text(std::string("Remain: ") + std::to_string(weapon->ammo()), 1, g_Render->font.ESPMini, Color(246, 0, 129, 255)));
				}
			}
